...
! <<^V^\\=========================================//-//-//========//O\\//
  SUBROUTINE energy_tetra(grid, xq0, S, fc2, freqs_doubled_C, freqs_doubled_X)
    IMPLICIT NONE
    !
    ...
    INTEGER :: iq, iqp, ibnd, jbnd, index_double
    REAL(DP), allocatable, intent(out) :: freqs_doubled_X(:,:), freqs_doubled_C(:,:)
    REAL(DP) :: xq(3,3), freq(S%nat3,3)
    !
    allocate(freqs_doubled_C(S%nat3**2, grid%nqtot))
    allocate(freqs_doubled_X(S%nat3**2, grid%nqtot))
    !
    ...
    freqs_doubled_C = 0._dp
    freqs_doubled_X = 0._dp
    DO iq = 1, grid%nq
      iqp = iq + grid%iq0
      ...
      DO ibnd = 1, S%nat3
        DO jbnd = 1, S%nat3
          ...
          freqs_doubled_C(index_double,iqp) = freq(jbnd,3) - freq(ibnd,2)
          freqs_doubled_X(index_double,iqp) = freq(ibnd,2) + freq(jbnd,3)
        ENDDO
      ENDDO
    ENDDO
    call mpi_bsum(S%nat3**2, grid%nqtot, freqs_doubled_C)
    call mpi_bsum(S%nat3**2, grid%nqtot, freqs_doubled_X)
  ...
  FUNCTION linewidth_q(xq1, S, fc2, fc3, grid, input, lw_UN)
    ...
    INTEGER :: iq, jq, j_un, it, nu0(3), i,j,k, ii
    ...
    REAL(DP), allocatable :: freq_C(:,:), freq_X(:,:)
...
!$OMP END PARALLEL DO
          ...
          do ii = 1, S%nat3
            do j = 1,3
              IF(ii>=nu0(j)) freqm1(ii,j) = 0.5_dp/freq(ii,j)
            ENDDO
          enddo
          ...
          DO j = 1,S%nat3
            DO k = 1,S%nat3
              ...
              SELECT CASE(input%delta_approx)
               ...
               CASE ("tetra")
                ctm = bose_C * weights_C(S%nat3*(j-1) + k, iq+grid%iq0) + &
                & bose_X * weights_X(S%nat3*(j-1) + k, iq+grid%iq0)
               CASE DEFAULT
