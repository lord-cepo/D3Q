...
CONTAINS
  ...
  !
  !
  subroutine deallocate_dqr(this)
    class(d3_mixed), intent(inout) :: this
    !
    if (allocated(this%R3)) deallocate(this%R3)
...
!$OMP END PARALLEL DO
  ...
  SUBROUTINE ip_cart2pat_gemm(d3in, nat3, u1, u2, u3)
    ...
    CALL zgemm('N', 'N', nat32, nat3, nat3, zone, d3in, nat32, u3c, nat3, zzero, tmp, nat32)

    ! ! Step 2: tmp2(i,b,k) = TRANSCONJ(u(a,i)) * tmp(a,b,k)
    CALL zgemm('C', 'N', nat3, nat32, nat3, zone, u1, nat3, tmp, nat3, zzero, d3in, nat3)
    ...
    CALL zgemm('C', 'N', nat3, nat32, nat3, zone, u2, nat3, tmp, nat3, zzero, d3in, nat3)
