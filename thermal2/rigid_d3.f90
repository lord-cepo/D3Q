...
!  ENDIF
    ...
    do i = 1, 3
      IF (nopbc(i)) THEN
        nrx(i) = 0
      ELSE
        nrx(i) = INT(SQRT(geg) / norm2(bg(:,i))) + 1
      ENDIF
    enddo
    !
      ...
      DO m2 = -nrx(2), nrx(2)
        DO m3 = -nrx(3), nrx(3)
          ...
          IF (loto_2d) THEN
            ...
            grg = 0.0d0
            g2d = g(1:2)
            IF (norm2(g2d)**2 > 1.0d-8) THEN
              grg = dot_product(g2d, matmul(reff, g2d))
              grg = grg / norm2(g2d)**2
            ENDIF
          ELSE
...
!$OMP PARALLELDO DEFAULT(shared) PRIVATE(zcg,zag,arg,nb,na,i,j,fnat)
            DO na = 1, nat
              ...
              fnat(:) = 0.d0
              DO nb = 1, nat
                arg = 2.d0 * pi * dot_product(g, tau(:, na) - tau(:, nb))
                zcg(:)  = matmul(g, zeu(:, :, nb))
...
!$OMP PARALLELDO DEFAULT(shared) PRIVATE(zbg,zag,arg,nb,na,i,j,facg)
            DO nb = 1, nat
              ...
              DO na = 1, nat
                zag(:) = matmul(g, zeu(:, :, na))
                arg = 2 * pi * dot_product(g, tau(:, na) - tau(:, nb))
