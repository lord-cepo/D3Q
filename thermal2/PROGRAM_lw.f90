...
CONTAINS
  ...
  SUBROUTINE SPECTR_QBZ_LINE(input, qpath, S, fc2, fc3)
    ...
    DO iq = input%skip_q +1,qpath%nq
      ...
      !   print*, w2
      !   DO it = 1,input%nconf
      !     open(1000+it, file="self-energy-anh-01.dat")
      !     DO ie = 1, input%ne
      !       read(1000+it, '(19E25.8)') dummy, selfnrg(ie, :, it)
      !       DO i = 1,S%nat3
      !         gamma =  AIMAG(selfnrg(ie,i,it))
      !         delta =  REAL(selfnrg(ie,i,it), DP)
      !         spectralf(ie,i,it) = - gamma / ((dummy**2 - w2(i)**2 - delta)**2 + gamma**2)
      !       ENDDO
      !     ENDDO
      !   ENDDO
      !   open(123, file='spf.out', status='replace')
      !   DO ie = 1,input%ne
      !     write(123, '(100e25.8)') &
      !       ener(ie)*RY_TO_CMM1, SUM(spectralf(ie,:,1))*UNIT_CONVERSION, &
      !       spectralf(ie,:,1)*UNIT_CONVERSION
      !   ENDDO
      !   close(123)
