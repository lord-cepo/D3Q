...
contains
  ...
  subroutine main_defect(S, Sd, fc2, fc2d, grid, out_grid, input)
    ...
    complex(dp), dimension(S%nat3, S%nat3) :: Vqq, T_Us, temp, Vqq2, V2, V7, green
    ...
    complex(dp), dimension(S%nat3, S%nat3, out_grid%nqtot) :: out_Us
    complex(dp) :: tetra_weights(S%nat3, grid%nqtot, 0:input%n_omega)
    complex(dp), dimension(S%nat3, grid%nqtot) :: interp, interp2, interp7
    ...
    integer :: iR_def, na_def, i, j, jbnd, ibnd1, dim_deg, ir, jr, jqp ! , jn1, jn2
    ...
    if(full_born) allocate( V_flat(Nin,Nout))
    if(full_born) allocate(VG_flat, VG2_flat, VG3_flat, VG4_flat, source=V_flat)
    !
    ...
    !
    call freq_in_grid_degen(S, fc2_centered, fc2_sc, out_grid, out_freqs, out_Us, out_freqs1)
    call freq_in_grid_degen(S, fc2_centered, fc2_sc, grid, freqs, Us, freqs1)
    !
    ...
    do iq = 1, out_grid%nq
      ...
      do jq = 1, grid%nq
        call fc2_sc%interpolate(grid%xq(:,jq), S, Vqqs(:,:,jq))
      enddo
      ibnd = 1
      do !ibnd
        ...
        if (dim_deg > 1) then
          ...
          lws(ibnd:ibnd+dim_deg-1,iqp) = e2(ibnd:ibnd+dim_deg-1) / omegaq / grid%nqtot
        else
          lws(ibnd,iqp) = braket(out_us(:,ibnd,iqp),Vqq2) / omegaq / grid%nqtot
        endif
        ibnd = ibnd + dim_deg
        if(ibnd > S%nat3) exit
      enddo
    ...
    if(full_born) then
      ...
      do iq = 1, grid%nq
        ...
        T_Us = CONJG(TRANSPOSE(out_Us(:,:,iqp)))
        do jq = 1, out_grid%nqtot
          ...
          ! call zgemm('C', 'N', S%nat3, S%nat3, S%nat3, 1._dp, out_Us(:,:,iqp), &
      ...
      do iw = my_id, input%n_omega, num_procs
        ...
        ! VG_flat = Id_flat - VG_flat
        ! call invzmat(S%nat3*out_grid%nqtot, VG_flat)
        ! ! VG_flat = VG_flat + matmul(VG_flat, VG_flat)
        ! VG_flat = matmul(V_flat, VG_flat)
        !
      ...
      do iq = 1, out_grid%nq
        do ibnd = 1, S%nat3
          ...
          lws_full(ibnd,iq) = interp1_scl(lws_full_iw(ibnd,iq,:), &
            omegaq*input%n_omega/max_freq) / omegaq / grid%nqtot
        enddo
      enddo
      !
    endif
    ...
    if(ionode) then
      ...
      do iq = 1, out_grid%nqtot
        do ibnd = 1, S%nat3
          write(10, "(3e14.5,I3,3F7.2)") out_freqs(ibnd,iq), lws(ibnd,iq), ibnd, out_grid%xq(:,iq)
        enddo
      enddo
      close(10)
      !
      ...
      do iq = 1, out_grid%nqtot
        do ibnd = 1, S%nat3
          write(10, "(3e14.5,I3,3F7.2)") out_freqs(ibnd,iq), lws_full(ibnd,iq), ibnd, out_grid%xq(:,iq)
        enddo
      enddo
      close(10)
    endif
    ...
    ! endif
    !
    if (full_born) deallocate(V_flat, VG_flat)
    ! deallocate(V, V_flat)
  end subroutine
  !
end module
